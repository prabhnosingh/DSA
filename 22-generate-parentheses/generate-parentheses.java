class Solution {
    //Re-sovling on 17 Mar 2026:

    //intuition 1 : dfs : Backtracking
        //both opening and closing brackets should be of n count
        //every combination must start with "("
        //we cannot close any bracket that is not opened yet, therefore,
            //keep track of opening brackets and make the option of
            //closing a bracket available only if the number of open 
            //brackets are greater than 0
        //open brackets should not exceed n. Closing brackets will
            //take care of itself
        //given the above constraints, we have two options for each
            //position, either to open the bracket or to close it.
        
        //TC: O(2^n) : we traverse two branches for n times
        //SC: O(2n) = O(n) : height of recursive stack
    public List<String> generateParenthesis(int n) {
        ...
        dfs(n, combs, new StringBuilder(), open, close);

        return combs;

    }

    private void dfs(int n, List<String> combs, StringBuilder currComb, int open,
    int close){
        if(open == n && close == n){
        ...
        if(open < n){
            currComb.append('(');
            dfs(n, combs, currComb, open + 1, close);
            currComb.deleteCharAt(currComb.length() - 1);
        }

        if(close < n && open != 0 && open != close){
            //do not close when open is 0
            //do not close when open is equal to close, since there should be
                //at least 1 open bracket to be closed
            currComb.append(')');
            dfs(n, combs, currComb, open, close + 1);
            currComb.deleteCharAt(currComb.length() - 1);
        }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //         //closing count and do not close a parentheses if there is more or equal closed parentheses 
