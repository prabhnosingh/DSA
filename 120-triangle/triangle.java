class Solution {
    ...
    //intuition 2 (Space optimized 1 D DP): 
        //As each cell depends only on below two cells for the path, we only have a 
            //single 1D dp array and update that only
    ...
    public int minimumTotal(List<List<Integer>> triangle) {
        ...
        //dpp[0][0] will represent minimum path sum to reach bottom level
            //from top
        //The most extreme cell that can exist is triangle.size()-1, 
            //triangle.get(triangle.size()-1).size()-1
        //Therefore we need a 2D DP matrix of size triangle.size() x
            //triangle.get(triangle.size()-1).size()

        
        int rows = triangle.size();
        int cols = triangle.get(rows-1).size(); //cols will be length of longest list in triangle
        ...
        for(int i = rows - 2; i >= 0; i --){
            // for(int j = i; j >= 0; j --){ //cannot do this because by running this loop
                //we will be considering new states while computing current states instead
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //     //Therefore, problem reduces to finding next two states to find the current state.
